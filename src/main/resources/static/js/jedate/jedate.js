	...
	*/
    ...
    InitDate.prototype = {
        init:function() {
            ...
            var nowDateVal = jeDt.nowDate(new Date()-opts.initHour*60*60*1000*-1, opts.format);
            if (opts.isinitVal) {
        ...
        setHtml:function(opts, self) {
            var that = this, weekHtml = "", date = new Date(), nowDateVal = jeDt.nowDate(new Date()-opts.initHour*60*60*1000*-1, opts.format), isformat = opts.format.match(/\w+|d+/g).join("-") == "YYYY-MM" ? true :false;
...
function fmDate(timestamp, format) {
    var De = new Date(timestamp | 0 ? function(tamp) {
        return tamp < 864e5 ? +new Date() + tamp * 864e5 :tamp;
    }(parseInt(timestamp)) :+new Date());
    return parse([ De.getFullYear(), De.getMonth() + 1, De.getDate() ], [ De.getHours(), De.getMinutes(), De.getSeconds() ], format);
};
function parse(ymd, hms, format) {
    ymd = ymd.concat(hms);
    var format = format, _this = this;
    return format.replace(/YYYY|MM|DD|hh|mm|ss/g, function(str, index) {
        ymd.index = ++ymd.index | 0;
